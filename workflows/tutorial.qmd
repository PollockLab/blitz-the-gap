...
base.5k = terra::rast(here::here("data/raw/base-layers/canada.base.5k.tiff"))

# Mask water and built areas
base.water <- rast(here::here("data/raw/base-layers/WaterUrbanBuiltMask.tif"))
base.water<-project(base.water,crs(base.5k))
base.water<-resample(base.water,base.5k,method='near')
terra::writeRaster(base.water, here::here("data/processed/base-layers/WaterUrbanBuiltMask.tif"))
...
terra::writeRaster(anti.base, here::here("data/processed/base-layers/anti.base.5k.tif"))
```
...
gbif = data.table::fread(here::here("data/raw/gbif/amphibians/0007149-250127130748423.csv")) |>
...
# save
writeRaster(gbif.layers, here::here("data/processed/gbif.layers.amphibians.tif"), overwrite = TRUE)
writeVector(gbif.v, here::here("data/processed/gbif.points.amphibians.shp"), overwrite = TRUE)
```
...
```{r, eval = F}
vel <- rast(here::here("data/raw/base-layers/climatevelocity_adaptwest_fwvel_ensemble_rcp85_2085.tif"))
vel<-project(vel,base.5k,"bilinear")
vel<-resample(vel,base.5k,method='near')
vel[is.na(base.5k)] <- NA
terra::writeRaster(vel, here::here("data/processed/base-layers/climatevelocity_adaptwest_fwvel_ensemble_rcp85_2085.tif"))
```
...
```{r, eval = F}
dist.to.roads<-rast(here::here("data/raw/base-layers/distance.to.roads.tif"))
dist.to.roads<-project(dist.to.roads,base.5k,"bilinear")
dist.to.roads[is.na(base.5k)]<-NA
terra::writeRaster(dist.to.roads, here::here("data/processed/base-layers/dist.to.roads.tif"))
```

```{r, echo = FALSE, include=FALSE}
# hidden chunk to load datasets prepared with the code above
gbif.layers = terra::rast(here::here("data/processed/gbif.layers.amphibians.tif"))
gbif.v = terra::vect(here::here("data/processed/gbif.points.amphibians.shp"))
vel = terra::rast(here::here("data/processed/base-layers/climatevelocity_adaptwest_fwvel_ensemble_rcp85_2085.tif"))
dist.to.roads = terra::rast(here::here("data/processed/base-layers/dist.to.roads.tif"))
```
